"""jarvis-code — the assistant's work, visible on the strip.
...
# The jarvis platform, hard-coded for the reason `jarvis_vision`
...
def watch(path: Path, stop: threading.Event) -> None:
    ...
    logger.info(f"jarvis-code: mirando {path}")
...
def register(ctx):
    ...
    if bridge:
        threading.Thread(
            ...
            name="jarvis-code-bridge",
            daemon=True,
        ).start()
        return

    path = Path(os.environ.get("JARVIS_CODE_LIVE", "") or DEFAULT_LIVE).expanduser()

    def run() -> None:
        try:
            watch(path, stop)
        except Exception as exc:
            logger.warning(f"jarvis-code: el seguidor se detuvo — {exc}")

    threading.Thread(target=run, name="jarvis-code-live", daemon=True).start()
        ...
        for event in client.follow_events(bridge, stop.is_set):
            try:
                ...
                logger.opt(exception=True).warning(
                    f"jarvis-code: evento descartado — {exc}"
                )
    except Exception as exc:  # noqa: BLE001 — the follower owns its failure
        logger.opt(exception=True).warning(
            f"jarvis-code: el modo puente se detuvo — {exc}"
        )
    finally:

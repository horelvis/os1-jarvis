    ...
    *,
    ...
    def _detections(frame: Any, camera: str) -> list:
        ...
        try:
            ...
            logger.warning(f"jarvis-vision: {camera} detect failed — {redact(exc)}")
            return []
    ...
    async def _show(frame: Any, camera: str) -> None:
        ...
        try:
            ...
            logger.warning(
                f"jarvis-vision: {camera} snapshot not written — {redact(exc)}"
            )
            return
        try:
            if not await push_photo(str(path), camera):
                logger.debug(f"jarvis-vision: {camera} photo not shown")
        except Exception as exc:
            # push_photo promises never to raise. Believing a promise is
            # not the same as depending on one, and the gateway is the
            # brain.
            logger.warning(f"jarvis-vision: {camera} photo not shown — {redact(exc)}")
    ...
    async def _vote(camera: str) -> tuple[Any, list]:
        ...
        for index in range(VOTE_FRAMES):
            ...
            try:
                ...
                logger.warning(f"jarvis-vision: {camera} grab failed — {redact(exc)}")
                frame = None
            if frame is None:
    ...
    async def handler(args: Any = None, **_kwargs: Any) -> str:
        ...
        logger.info(f"jarvis-vision: mirar {wanted or '(todas)'}")

    ...
    *,
    ...
    for index, delay in enumerate((0.0, *delays)):
        ...
        try:
            ...
            logger.warning(f"jarvis-vision: injection raised — {redact(exc)}")
            return False
        if accepted:
            if index:
                logger.debug(f"jarvis-vision: delivered on attempt {index + 1}")
            return True
    ...
    logger.warning(
        "jarvis-vision: no live gateway, sighting dropped after "
    ...
    *,
    ...
    def on_detections(camera_name: str, detections: list, frame: Any = None) -> None:
        try:
            ...
            if not phrase:
                return
            logger.info(f"jarvis-vision: {camera_name}: {phrase}")
            ...
            if show is not None and frame is not None:
                try:
                    ...
                    logger.warning(
                        f"jarvis-vision: {camera_name}: photo not shown — "
                        f"{redact(exc)}"
                    )
            ...
            logger.warning(
                f"jarvis-vision: {camera_name}: alert failed — {redact(exc)}"
            )

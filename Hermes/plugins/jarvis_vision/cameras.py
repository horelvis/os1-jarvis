...
def parse_cameras(cfg: dict[str, Any]) -> list[Camera]:
    ...
    if not isinstance(raw, list):
        ...
        logger.warning(f"jarvis-vision: 'cameras' is not a list: {redact(repr(raw))}")
        raw = []
    out: list[Camera] = []
    seen: set[str] = set()
    for entry in raw:
        if not isinstance(entry, dict):
            ...
            logger.warning(
                f"jarvis-vision: camera entry is not a mapping: {redact(repr(entry))}"
            )
            continue
        name = entry.get("name")
        url = entry.get("url")
        if not name or not url:
            logger.warning(
                "jarvis-vision: camera entry without name or url: "
                f"{redact(repr(entry))}"
            )
            continue
        if name in seen:
            logger.warning(
                f"jarvis-vision: duplicate camera name {name!r}, keeping the first"
            )
            continue
        ...
        if expanded is None:
            ...
            logger.warning(
                f"jarvis-vision: camera {str(name)!r} dropped, "
    ...
    if not out:
        logger.info(
            "jarvis-vision: no cameras configured (config key 'cameras' empty)"
        )
    return out
...
class CameraFleet:
    ...
    def start(
        ...
        try:
            ...
            logger.error(
                f"jarvis-vision: no detector, no cameras watched — {redact(exc)}"
            )
            return
        ...
        for camera in cameras:
            ...
            thread.start()
        logger.info(
            f"jarvis-vision: watching {len(cameras)} camera(s): "
    ...
    def stop(self, timeout: float = JOIN_TIMEOUT) -> None:
        ...
        for thread in threads:
            ...
            if thread.is_alive():
                logger.debug(f"jarvis-vision: {thread.name} still in a read, left")
            ...
            try:
                ...
                for frame in stream.frames(
                    ...
                    if reported_unreachable or reported_empty:
                        logger.info(f"jarvis-vision: {camera.name} is back")
                    ...
                    try:
                        ...
                        logger.debug(
                            f"jarvis-vision: {camera.name} offer failed — "
                            f"{redact(exc)}"
                        )
                ...
                if frames_seen == 0 and not self._stopping.is_set():
                    if not reported_empty:
                        logger.warning(
                            f"jarvis-vision: {camera.name} connected but "
                        ...
                        logger.debug(
                            f"jarvis-vision: {camera.name} still producing no frames"
                        )
            except Exception as exc:
                ...
                if not reported_unreachable:
                    logger.warning(
                        f"jarvis-vision: {camera.name} unreachable — {redact(exc)}"
                    )
                    reported_unreachable = True
                    reported_empty = False
                else:
                    logger.debug(f"jarvis-vision: {camera.name} still unreachable")
            finally:
                ...
                if stream is not None:
                    try:
                        ...
                        logger.debug(f"jarvis-vision: {camera.name} would not close")
    ...
    def _report(self, camera: Camera, detector: Any, frame, on_detections) -> None:
        ...
        try:
            ...
            logger.debug(
                f"jarvis-vision: {camera.name} detect failed — {redact(exc)}"
            )
            return
        if not seen:
            return
        try:
            ...
            logger.warning(
                f"jarvis-vision: {camera.name} handler failed — {redact(exc)}"
            )

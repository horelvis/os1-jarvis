...
class LiveSession:
    ...
    async def open(
        ...
        try:
            ...
            logger.warning(f"jarvis-vision: live_open not delivered — {redact(exc)}")
            return False
        if not opened:
        ...
        logger.debug(
            f"jarvis-vision: tap installed for {camera} (epoch {self.epoch})"
        )
        return True
    ...
    async def close(self, reason: str) -> bool:
        ...
        logger.debug(
            f"jarvis-vision: {camera} closed after {self._packets} packets ({reason})"
        )
        try:
            await self._push_close(self.epoch, reason)
        except Exception as exc:
            logger.warning(f"jarvis-vision: live_end not delivered — {redact(exc)}")
        return True
    ...
    def _on_packet(self, camera: str, packet: bytes, keyframe: bool) -> None:
        ...
        if self._packets == 1:
            logger.debug(
                f"jarvis-vision: first packet from {camera} (keyframe={keyframe})"
            )
        ...
        if not self._keyframe_seen:
            ...
            logger.debug(
                f"jarvis-vision: streaming {camera} from packet {self._packets}"
            )
    ...
    def _schedule(self, coro: Coroutine[Any, Any, bool]) -> None:
        ...
        if loop is None or loop.is_closed():
            ...
            if not self._loop_reported:
                ...
                logger.warning(
                    "jarvis-vision: no loop to deliver on "
        ...
        if not self._loop_reported:
            self._loop_reported = True
            logger.debug(
                f"jarvis-vision: delivering on loop {id(loop):x} "
                f"(running={loop.is_running()})"
            )
        try:
            asyncio.run_coroutine_threadsafe(coro, loop)
        except RuntimeError as exc:
            logger.debug(f"jarvis-vision: live frame not scheduled — {exc}")
            coro.close()

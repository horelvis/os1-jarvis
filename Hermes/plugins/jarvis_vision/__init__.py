"""jarvis-vision — the house cameras, and what is worth saying."""
...
def register(ctx):
    ...
    logger.info("jarvis-vision: registered")
...
def _supervise(ctx, fleet: CameraFleet, names: list[str] | None = None) -> None:
    ...
    try:
        ...
        logger.error(f"jarvis-vision: cameras not started — {redact(exc)}")
...
async def push_photo(path: str, camera: str) -> bool:
    ...
    try:
        ...
        if runner is None:
            logger.debug("jarvis-vision: no gateway, photo dropped")
            return False
        adapter = getattr(runner, "adapters", {}).get(Platform(JARVIS_PLATFORM))
        if adapter is None:
            logger.debug("jarvis-vision: no strip platform, photo dropped")
            return False
        ...
        logger.warning(f"jarvis-vision: photo not shown — {redact(exc)}")
        return False
...
def _show_sighting(frame: Any, camera: str) -> None:
    ...
    if loop is None or loop.is_closed():
        logger.debug(f"jarvis-vision: {camera} photo not shown — no loop")
        return
...
async def push_live_open(
    ...
    try:
        ...
        logger.warning(f"jarvis-vision: live not opened — {redact(exc)}")
        return False
...
def _note_delivery(epoch: int, ok: bool, exc: BaseException | None = None) -> None:
    ...
    if exc is not None:
        logger.warning(f"jarvis-vision: first frame raised — {redact(exc)}")
    else:
        logger.debug(f"jarvis-vision: first frame of epoch {epoch} landed={ok}")
...
async def push_live_close(epoch: int, reason: str) -> bool:
    ...
    try:
        ...
        logger.warning(f"jarvis-vision: live not closed — {redact(exc)}")
        return False

...
### Task 4.6: OS1Loader port (Three.js cinta from spec §5.1)

**Goal:** Port `backend/static/os1-loader.js` (the rotating ribbon from the film, attributed to Siyoung Park MIT) to a React component, and use it in the BootScreen. Onboarding (Phase 8) can reference it later if we expand to the full cinematic flow; v2 ships with the cinta visible at least at boot.

**Files:**
- Create: `frontend/src/components/OS1Loader.tsx`
- Modify: `frontend/src/screens/BootScreen.tsx`

- [ ] **Step 1: Write `OS1Loader.tsx`**

This is a near-mechanical port of `backend/static/os1-loader.js` wrapped in a React component. The component takes `size` (small | medium | large) and `startTransformed` (boolean) props, mounts a Three.js renderer in a div, and exposes an imperative handle to call `transform()` / `untransform()` / `setActive()` from outside.
...
- [ ] **Step 2: Update `BootScreen.tsx` to render OS1Loader**

Replace the body of `BootScreen.tsx`:
...
Expected: no errors. If `three` types complain, ensure `@types/three` is in devDependencies (already added in Task 4.1).

- [ ] **Step 4: Smoke test**

`npm run dev`. Open localhost:5173/. The rotating ribbon should appear centered above the "samantha" label, then transition away when navigation kicks in.

- [ ] **Step 5: Commit**

```bash
git add frontend/src/components/OS1Loader.tsx frontend/src/screens/BootScreen.tsx
git commit -m "feat(frontend): OS1Loader Three.js port + BootScreen integration"
```
...
- `npm run dev` shows Boot (cinta + samantha) → placeholder based on profile

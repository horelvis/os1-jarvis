...
- **A rigged model** — glTF or USDZ — becomes an asset this project has
  to source, license and keep. An image generated by a text-to-image
  tool is a visual reference and **not** an asset: without a rig it
  cannot move. This is the real cost, and it is a sourcing problem
  rather than an engineering one.
- **Lip-sync is the cheap part, and an earlier draft of this spec was
  wrong to imply otherwise** (corrected the same day, at the user's
  push-back: *"hay miles de juegos de iPhone que usan un personaje
  charlando"* — and they are right). Drive the jaw from the amplitude
  of the 24 kHz PCM the app already receives: a running RMS mapped to
  one blendshape, tens of lines, which is what those games do.
  **Viseme-level movement is only needed for a realistic human mouth**,
  where the uncanny valley punishes a mismatch — and the whole point of
  choosing a stylised non-human character is that it does not.
  Implement the amplitude floor; do not reach for a phoneme model
  unless something measured says it is needed.
- **Rendering is commodity.** SceneKit or RealityKit animates a rigged
  model on an iPhone with no invention required. Nothing in this
  section is novel work; it is assembly.
